...
<svg {{ $attributes->merge([
    'viewBox' => $isPhone ? '0 0 32 32' : '0 0 24 24',
    'fill' => $isPhone ? 'currentColor' : 'none',
    'xmlns' => 'http://www.w3.org/2000/svg',
    'aria-hidden' => 'true',
]) }}>
    @switch($name)
        @case('package')
            {{-- Closed box with lid seam --}}
            <path d="M5 9.5 12 6l7 3.5v8L12 21l-7-3.5v-8Z" stroke="{{ $stroke }}" stroke-width="{{ $sw }}" stroke-linejoin="round" />
            <path d="M5 9.5 12 13l7-3.5" stroke="{{ $stroke }}" stroke-width="{{ $sw }}" stroke-linecap="round" stroke-linejoin="round" />
            <path d="M12 13v8" stroke="{{ $stroke }}" stroke-width="{{ $sw }}" stroke-linecap="round" />
            @break

        @case('map')
            {{-- Folded map --}}
            <path d="M5 6.5 9 5l5 2 5-2v13l-5 2-5-2-4 1.5V6.5Z" stroke="{{ $stroke }}" stroke-width="{{ $sw }}" stroke-linejoin="round" />
            <path d="M9 5v13" stroke="{{ $stroke }}" stroke-width="{{ $sw }}" stroke-linecap="round" />
            <path d="M14 7v13" stroke="{{ $stroke }}" stroke-width="{{ $sw }}" stroke-linecap="round" />
            @break

        @case('addons')
            {{-- Three circles + plus in 2x2 grid --}}
            <circle cx="8.25" cy="8.25" r="2.25" stroke="{{ $stroke }}" stroke-width="{{ $sw }}" />
            <circle cx="15.75" cy="8.25" r="2.25" stroke="{{ $stroke }}" stroke-width="{{ $sw }}" />
            <circle cx="8.25" cy="15.75" r="2.25" stroke="{{ $stroke }}" stroke-width="{{ $sw }}" />
            <path d="M15.75 13.5v4.5M13.5 15.75h4.5" stroke="{{ $stroke }}" stroke-width="{{ $sw }}" stroke-linecap="round" />
            @break
